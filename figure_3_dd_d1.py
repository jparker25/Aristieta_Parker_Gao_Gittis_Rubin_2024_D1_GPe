"""
File: figure_3_dd_d1.py
Author: John E. Parker
Date: 13 February 2024

Description: Generates figure 3 based d1 dd data and compares with d1 naive data.
"""
...
def generate_figure(data_short, compare_short, save_dir):
    """
    Generates figure 3 with d1 dd data set, compared with d1 naive data, and displays figure.

    data_short\t:\tdataframe of d1 dd data
    ...
    psth_norm_compare(
        ...
        semcolor2="gray",
        drawboth=True,
        label1=f"D1-DD ($n={len(data_short)}$)",
        label2=f"D1-ctl ($n={len(compare_short)}$)",
        lw=1,
    )
    ...
    # Plot modulation factors for FR and CV, compare with d1 naive MFs (remove complete inhibition for CV)
    category_change_MF(
        compare_short,
        data_short,
        "baseline_freq",
    ...
    # Plot distribution of responses, compare with d1 naive data
    classification_plot_comparison_horizontal(
        data_short, compare_short, classifications[0], "D1-DD", "D1-ctl", plot_vals=True
    )
    ...
    plt.savefig(
        f"{save_dir}/figure_3.pdf",
        bbox_inches="tight",
        pad_inches=0.0,
    )
    plt.close()
    run_cmd(f"open {save_dir}/figure_3.pdf")
